...
// in assignment expressions
void type_class_member_jump_check(struct tree *t, int assn_type)
{
    int i; 
    int type1; 
	//int type2; 
	...
	for(i=0; i < TABLE_SIZE; i++)
	{
		if(class_tables[i])
		{
			if(find_sym_in_list(t->kid[1]->kid[0]->kid[2]->leaf->text, class_tables[i]->name))
				{
					type1 = find_type_in_list(t->kid[1]->kid[0]->kid[2]->leaf->text, class_tables[i]->name);
					//curr_class_name = strdup(class_tables[i]->name); 
					//printf("Found type! %s, %d, %s\n", t->kid[1]->kid[0]->kid[2]->leaf->text, type1, curr_class_name); 
					if(type1 != assn_type)
					{
						semanticerror("Class function does not match assignment type:", t->kid[1]->kid[0]->kid[2]); 
					}
				}
		}
...
// in assignment expressions
void type_class_member_jump_2_check(struct tree *t, int assn_type)
{
    int i; 
    int type1; 
	//int type2; 
	...
	for(i=0; i < TABLE_SIZE; i++)
	{
		if(class_tables[i])
		{
			if(find_sym_in_list(t->kid[1]->kid[2]->leaf->text, class_tables[i]->name))
				{
					type1 = find_type_in_list(t->kid[1]->kid[2]->leaf->text, class_tables[i]->name);
					//curr_class_name = strdup(class_tables[i]->name); 
					//printf("Found type! %s, %d, %s\n", t->kid[1]->kid[2]->leaf->text, type1, curr_class_name); 
					if(type1 != assn_type)
					{
						semanticerror("Class function does not match assignment type:", t->kid[1]->kid[2]); 
					}
				}
		}
